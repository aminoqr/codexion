_This project has been created as part of the 42 curriculum by aasylbye._
...
The codebase is annotated with a global numbering scheme `[NN]`. Numbers are
sequential, starting at the type definitions in the header and ending in
`cleanup.c`. Reading the numbered comments in ascending order walks you
through the codebase the same way it was authored:

| Range | File | Topic |
| --- | --- | --- |
| `[01]..[08]` | `include/codexion.h` | Types, enums, public API |
| `[09]..[10]` | `src/main.c`         | Entry point + thread orchestration |
| `[11]..[15]` | `src/parse.c`        | Argument parsing & validation |
| `[16]..[18]` | `src/time.c`         | Wall-clock helpers, cooperative sleep |
| `[19]..[21]` | `src/log.c`          | Mutex-serialised logging |
| `[22]..[24]` | `src/heap.c`         | Heap lifecycle |
| `[25]..[29]` | `src/heap_ops.c`     | Heap push / pop with FIFO+EDF comparator |
| `[30]..[34]` | `src/init.c`         | Allocation & sync primitives |
| `[35]..[39]` | `src/dongle.c`       | Dongle acquire / release / arbitration |
| `[40]..[44]` | `src/coder.c`        | Coder thread loop and phases |
| `[45]..[48]` | `src/monitor.c`      | Burnout watchdog thread |
| `[49]..[52]` | `src/cleanup.c`      | Symmetric tear-down |
...
- **Deadlock prevention via resource ordering** (breaks Coffman #4 — circular
  wait). In `coder_compile` (`src/coder.c`, `[42]`) every coder always
...
- **Starvation prevention** (FIFO and EDF fairness).
  ...
  be granted the dongle (`dongle_can_grant` in `[36]`). Under EDF, the queue
...
- **Cooldown handling.**
  After every release, `dongle_release` arms `cooldown_until_ms = now_ms() +
  dongle_cooldown` and broadcasts on the dongle's condition variable
  (`[39]`). `dongle_can_grant` refuses to hand the dongle out until that
  timestamp is reached, even if a waiter is at the heap top, satisfying the
  mandatory cooldown rule.

- **Precise burnout detection.**
  A dedicated **monitor thread** (`src/monitor.c`, `[48]`) polls every coder
...
- **Log serialisation.**
  ...
  `[19]..[21]`), which take `sim->log_lock` before each `printf` and `fflush`
  call. As a result no two messages can interleave on the same line.

- **Clean shutdown.**
  `broadcast_stop` (`[47]`) sets `sim->stop = 1` and broadcasts every
  ...
  `precise_sleep_ms` (`[18]`) re-checks `sim_should_stop` between sleep
  slices so coders mid-phase also exit promptly.

- **No memory leaks.**
  `sim_cleanup` (`[52]`) symmetrically tears down every mutex, condition
...
is enforced everywhere: only `compute_deadline` (`[35]`) takes
...
once by `broadcast_stop` (`[47]`). After flipping the flag, the monitor
...
- **`finished_count`**: incremented by `compile_finalize` (`[41]`) under
  `finished_lock`; read by `all_finished` (`[46]`) under the same lock.
...
AI was **not** used to write the threading core (acquire/release/monitor) end
to end; that was authored manually so I can defend every line during the
peer-evaluation. Every numbered comment marks code I can explain without
notes.
